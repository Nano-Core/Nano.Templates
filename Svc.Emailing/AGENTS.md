...
just the transient endpoint — refresh is the exception: `/auth/login/refresh` and the transient
external-login refresh never accept claims/roles from the caller at all, they're recovered from a
manifest claim embedded at login (`ClaimTypesExtended.TransientClaims`/`TransientClaimsExtensions`),
so a refresh can never grant more than the original login already did.
